...
export class AuditorComponent implements OnInit {
  ...
  onSelectAuditList(aud:any) {
    if(this.headActive == 1 || this.headActive == 2){
      this.router.navigate(['/app/auditor/ques'], { queryParams: { 'eng_id':aud.engagementId, 'storeId':aud.storeId,'store_name': aud.storeName,'status':this.status} });
      ...
      this.router.navigate(['/app/auditor/dealer-review'], { queryParams: { 'auditId': aud.audit_pkId, 'clientId': aud.client_pkId, 'eng_id': aud.engagementId, 'storeId': aud.storeId, 'store_name': aud.storeName } });
    }
